package Rule.Action.ConditionAction;

import Context.Context;
import Entity.definition.EntityDefinition;
import PRD.PRDAction;
import PRD.PRDElse;
import Property.definition.EnvPropertyDefinition;
import Rule.Action.Action;
import Rule.Action.ActionFactory;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

public class Else{
    private List<Action> actionList;

    public Else(PRDElse prdElse, HashMap<String, EntityDefinition> entities, HashMap<String, EnvPropertyDefinition> environmentProperties) {
        if (prdElse!=null){
            this.actionList = new ArrayList<>();
            for (PRDAction action : prdElse.getPRDAction()){
                actionList.add(ActionFactory.ActionCreator(action, entities, environmentProperties));
            }
        }
    }

    public List<Action> getActionList() {
        return actionList;
    }

    public void InvokeElse(Context context) {
        for (Action action : actionList) {
            action.Activate(context);
        }
    }
    public Integer getActionListSize() {
        return actionList.size();
    }
}
